<script lang="ts">
    ...
    interface Props {
        reviews?: any[];
        product?: string;
        initialData?: any; // To restore state if needed (optional enhancement)
    }
    ...
    function toggleHelpful(id: string) {
        ...
        votes = { ...votes };
        // Dispatch change immediately so parent knows current state
        dispatch('change', { votes });
    }
    
    // Initial dispatch (empty)
    dispatch('change', { votes: {} });
</script>
